...
extern void InitializeCOD4Hooks();
extern void UninitializeCOD4Cheater();
	...
	case DLL_PROCESS_ATTACH:
	{
		if (!IsTrayOpen())
		{
			...
			if (Initialize() == ERROR_SUCCESS)
			{
				...
				if(!isDevkit)
					removeFromList(Handle.hME);
			}
			else
			{
				DbgPrint("NiNJA Engine Failed To Start, Unloading!");
				*(WORD*)((DWORD)Handle.hME + 64) = 1;
				return FALSE;
			}
		}
		else
