...
void SetupMenuText()
{
	...
	MenuNames.push_back(E_ALLOC("Ninja = Trash"));
...
void DrawAd()
{
	...
	FLOAT X, Y;
		...
		CL_DrawTextPhysical_Ghosts("^5Welcome to ^2Ninjas V1.0 ^7cracked engine ^2| ^5Enjoy using this piece of shit menu owned by a ^2faggot ^5that scams", 0x7FFFFFFF, BigDevFont, MovementBO2SP, 40, 1.3, 1, 0, Vector4(1, 1, 1, 1), 0);
...
void DrawMenuControlsText()
{
	...
	if (!GetMenuOptionValue(MENU_SETTINGS, OPTION_DISABLEMENUINSTRUCTIONS))
	{
		if (!CheckInGame())
		{
			...
			sprintf(Output, "Press \x0016 & \x0012 To %s ^7Shit Engine | ^2#StillNoPickles", menuOpen ? "^1Close" : "^2Open");
			DrawText_Ghosts(Output, (X - 380), Y - 20, 0, 0, HudSmallFont, Vector4(1, 1, 1, 0.95f));
		...
		{
			sprintf(Output, "Press \x0016 & \x0012 To %s ^7Shit Engine | ^2#StillNoPickles", menuOpen ? "^1Close" : "^2Open");
			DrawText_Ghosts(Output, (cg_Ghosts->refdef.Width - 1250), cg_Ghosts->refdef.Height - 70, 0, 0, HudSmallFont, Vector4(1, 1, 1, 0.95f));
		}
	}
}
...
void R_EndFrameHook()
{
	...
	if (!inGame && menuReady)
	{
		if (!GetMenuOptionValue(MENU_SETTINGS, OPTION_DISABLENINJATEXT))
		{
			...
			//X = (FLOAT)*(DWORD*)getCryptedCoD(CheatPatches->CoDPatches.CG_S_XY);
			//Y = (FLOAT)*(DWORD*)(getCryptedCoD(CheatPatches->CoDPatches.CG_S_XY) + 0x4);
			//DrawText_Ghosts("Jester Likes Kids", ((X - 130) - (R_TextWidth_Ghosts("Jester Likes Kids", 0x7FFFFFFF, SmallFont) / 2) * 0.5f), (Y / 2) - 330, 0, 0, SmallFont, Vector4(0.102f, 1, 0.102f, 0.50f), 0.5f, 0.5f);
