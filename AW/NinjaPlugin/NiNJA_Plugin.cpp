...
BOOL unloadSysDLL() {
	HANDLE mHandle = GetModuleHandle("AW.xex");
	if (!mHandle) {
		DbgPrint("Failed to get handle to AW.xex");
		return FALSE;
	}
	*(WORD*)((DWORD)mHandle + 64) = 1;
	...
	{
		DWORD dwThreadId;
		LPCWSTR msg = L"NiNJA Engine [AW] Unloading!";
		...
		XNotifyUI_Title(L"NiNJA Engine [AW] Unloading!");
	}
	Sleep(3000);
	...
	while (!g_killThread) {
		...
		if ((State.Gamepad.wButtons & (WORD)((WORD)0 | (WORD)XINPUT_GAMEPAD_DPAD_LEFT | (WORD)XINPUT_GAMEPAD_RIGHT_THUMB)) == (WORD)((WORD)0 | (WORD)XINPUT_GAMEPAD_DPAD_LEFT | (WORD)XINPUT_GAMEPAD_RIGHT_THUMB))
		{
			if(!isHooked)
			{
				...
				XNotifyUI(L"NiNJA Engine [AW] Initialized!");
...
{	
	...
	case DLL_PROCESS_ATTACH:
	{
		if (!IsTrayOpen())
		{
			...
			{
				DbgPrint("NiNJA Engine [AW] Incorrect Title Id, Unloading!");
			...
			{
				DbgPrint("NiNJA Engine [AW] is ready!");
			}
			else
			{
				DbgPrint("NiNJA Engine [AW] Failed To Start, Unloading!");
				*(WORD*)((DWORD)hModule + 64) = 1;
				return FALSE;
			}
		}
		...
		{
			DbgPrint("NiNJA Engine [AW] Load Aborted, Tray Open...");
		}
		break;
	}
